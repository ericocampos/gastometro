...
async function main() {
  ...
  if (existsSync(alpbDeputados)) {
    interface DepAlpb { politicoId: string; nomeRegistro: string; nomeParlamentar?: string; partido?: string; fotoUrl?: string }
    ...
    for (const d of deps) {
      todosPoliticos.push({
        ...
        partido: d.partido ?? '—', uf: cfg.uf, legislaturas: [], fotoUrl: d.fotoUrl,
      })
