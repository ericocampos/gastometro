...
import { parseRoster, parseDespesas, montarDespesas, parseLotacoes, montarGabinetes, casarFotoTse, type DeputadoAlesp, type GabineteAlesp } from './sources/alesp.js'
...
async function main() {
  ...
  // 2) despesas itemizadas (>= ANO_MIN). XML grande: parse pesado, mas filtrado ja corta a maior parte.
  const recs = parseDespesas(await baixarXml('despesas', URL_DESPESAS), ANO_MIN)
