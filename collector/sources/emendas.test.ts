...
function reg(o: { codigo?: string; ano: string; tipo: string; codAutor: string; autor: string; mun: string; uf: string; funcao: string; emp: string; pago: string }) {
...
describe('parseEmendas', () => {
  const csv = [
    HEADER,
    reg({ codigo: '202412460001', ano: '2024', tipo: 'Emenda Individual - Transferências com Finalidade Definida', codAutor: '1246', autor: 'JULIO CESAR', mun: 'João Pessoa', uf: 'PB', funcao: 'Saúde', emp: '1.000.000,50', pago: '400.000,00' }),
    reg({ codigo: '202212460001', ano: '2022', tipo: 'Emenda Individual - Transferências com Finalidade Definida', codAutor: '1246', autor: 'JULIO CESAR', mun: 'João Pessoa', uf: 'PB', funcao: 'Saúde', emp: '999,99', pago: '0,00' }),
    reg({ codigo: '202470250001', ano: '2024', tipo: 'Emenda de Bancada', codAutor: '7025', autor: 'BANCADA DO ACRE', mun: 'Rio Branco', uf: 'AC', funcao: 'Urbanismo', emp: '2.000.000,00', pago: '1.000.000,00' }),
  ].join('\r\n')
  ...
  it('parseia campos, decimal BR e filtra por ano', () => {
    ...
    const r = regs[0]
    expect(r.codigo).toBe('202412460001')
...
const r = (o: Partial<RegistroEmenda>): RegistroEmenda => ({
  codigo: '', ano: 2024, tipo: 'Emenda Individual - Transferências com Finalidade Definida',
  autorCodigo: '0', autorNome: '', municipio: '', uf: '', funcao: '', empenhado: 0, pago: 0, ...o,
})

describe('agregarEmendas', () => {
  const regs: RegistroEmenda[] = [
    r({ codigo: 'E1', autorCodigo: '1246', autorNome: 'JULIO CESAR', municipio: 'João Pessoa', uf: 'PB', funcao: 'Saúde', empenhado: 1000, pago: 400 }),
    r({ codigo: 'E2', autorCodigo: '1246', autorNome: 'JULIO CESAR', municipio: 'Campina Grande', uf: 'PB', funcao: 'Educação', empenhado: 500, pago: 100 }),
  ...
  it('guarda a lista itemizada por emenda (uma por código, maior primeiro)', () => {
    const p = ag.porPolitico['camara-1']
    expect(p.emendas.length).toBe(2)
    expect(p.emendas[0]).toMatchObject({ codigo: 'E1', municipio: 'João Pessoa', funcao: 'Saúde', empenhado: 1000, pago: 400 })
    expect(p.emendas[1].codigo).toBe('E2')
  })
