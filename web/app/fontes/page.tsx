...
const BLOCOS: Bloco[] = [
  ...
  {
    casa: 'Câmaras no modelo completo via TCE (20 cidades: Santa Rita, Patos, Cabedelo, Sousa, Conde, Alhandra…)',
    intro: 'Onde a câmara não publica a VIAP de forma legível por máquina, a fonte primária passa a ser o próprio TCE-PB, que traz por vereador e por mês a VIAP como empenhos de "Indenizações e Restituições". Como o TCE é a fonte (não um cruzamento), não há selo de conferência. A VIAP nessas cidades é um valor fixo mensal (não reembolso de despesa itemizada) e varia muito de cidade para cidade: de ~R$ 1.000 a ~R$ 11.000 por vereador (de ~10% a ~100% do subsídio). Só entram cidades em que a maioria dos vereadores de fato recebe a VIAP (casável por CPF); sondamos as 212 câmaras leve e a maioria das pequenas não paga por essa via, seguindo no modelo simples.',
    fontes: [
